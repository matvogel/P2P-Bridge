...
class BallQuery(Function):
    ...
    def forward(ctx, radius, nsample, xyz, new_xyz, offset, new_offset):
        ...
        m = new_xyz.shape[0]
        idx = torch.cuda.IntTensor(m, nsample).zero_()
        pointops_cuda.ballquery_cuda(m, radius, nsample, xyz, new_xyz, offset, new_offset, idx)
        return idx
    ...
    if idx is None:
        if nsample is not None:
            if query_method in ["knn", "knnquery"]:
                ...
                idx = ballquery(radius, nsample, xyz, new_xyz, offset, new_offset)  # (m, nsample)
            ...
            if normalize_dp:
                max_dist = (
                    grouped_xyz.norm(dim=-1, p=2, keepdim=True).max(dim=-1, keepdim=True)[0] + 1.0e-8
                    if query_method == "knn"
                    else radius
                )
