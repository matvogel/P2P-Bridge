        ...
        **kwargs,
        ...
        if not os.path.exists(filename):
            ...
            for cloud_idx, cloud_name in enumerate(cloud_names):
                ...
                sub_cloud_file = os.path.join(processed_dir, cloud_name + f"_{voxel_size:.3f}_sub.pkl")
        ...
        if not os.path.exists(filename):
            ...
            for ep in tqdm(range(self.num_epochs)):
                for st in range(self.num_steps):
                    ...
                    points = np.array(self.sub_cloud_trees[cloud_ind].data, copy=False)  # sub_points
    ...
    def __getitem__(self, idx):
        ...
        if "heights" not in data.keys():
            data["heights"] = torch.from_numpy(
                original_points[:, self.gravity_dim : self.gravity_dim + 1].astype(np.float32)
            )
        return data
