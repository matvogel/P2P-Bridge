...
class MP40(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.uniform:
            self.save_path = os.path.join(root_dir, "matterport3dobjects_%s_%dpts_fps.h5" % (split, 2048))
        else:
            self.save_path = os.path.join(root_dir, "matterport3dobjects_%s_%dpts.h5" % (split, 2048))
        ...
        if not os.path.exists(self.save_path):
            ...
            for index in tqdm(range(len(self.datapath)), total=len(self.datapath)):
                ...
                if self.uniform:
                    point_set = fps(torch.from_numpy(point_set).unsqueeze(0).cuda(), npoints).squeeze(0).cpu().numpy()
                else:
