...
from third_party.PointNeXt.openpoints.models.layers import fps
...
class ScanObjectNNHardest(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        h5_name = os.path.join(data_dir, f"{slit_name}_objectdataset_augmentedrot_scale75.h5")

        if not osp.isfile(h5_name):
            raise FileExistsError(f"{h5_name} does not exist, please download dataset at first")
        ...
        if slit_name == "test" and uniform_sample:
            precomputed_path = os.path.join(data_dir, f"{slit_name}_objectdataset_augmentedrot_scale75_1024_fps.pkl")
        ...
        logging.info(
            f"Successfully load ScanObjectNN {split} " f"size: {self.points.shape}, num_classes: {self.labels.max()+1}"
        )
            ...
            data["x"] = torch.cat(
                (
                    ...
                    torch.from_numpy(
                        ...
                        - current_points[:, self.gravity_dim : self.gravity_dim + 1].min()
                    ),
                ),
                dim=1,
            )
