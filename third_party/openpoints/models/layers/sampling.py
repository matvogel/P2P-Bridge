...
class Gather(Function):
    ...
    def forward(ctx, features, indices):
        ...
        return pointnet2_cuda.gather_features_forward(features, indices)

    @staticmethod
    def backward(ctx, grad_output):
        (indices,) = ctx.saved_tensors
        grad_features = pointnet2_cuda.gather_features_backward(grad_output.contiguous(), indices, ctx.num_points)
        return grad_features, None


pvcnn_gather = Gather.apply


def furthest_point_sample_pvcnn(coords, num_samples, normals=None):
    assert len(coords.shape) == 3 and coords.shape[1] == 3, f"expect input as B,3,N; get: {coords.shape}"
    coords = coords.contiguous()
    indices = pointnet2_cuda.furthest_point_sampling_forward(coords, num_samples)
    centers_coords = pvcnn_gather(coords, indices)
    if normals is not None:
        center_normals = pvcnn_gather(normals, indices)
...
def logits_mask(coords, logits, num_points_per_object):
    ...
    for i in range(batch_size):
        ...
        if current_num_candidates >= num_points_per_object:
            ...
            selected_indices[i] = current_candidates[choices]
    selected_coords = pvcnn_gather(masked_coords - masked_coords_mean.view(batch_size, -1, 1), selected_indices)
