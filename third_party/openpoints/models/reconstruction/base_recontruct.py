...
from ..layers import create_linearblock, create_norm
from ..layers.attention import Block
from ..layers.graph_conv import DilatedKNN, GraphConv
...
class FoldingNet(nn.Module):
    ...
    def __init__(self, in_channels, emb_dims=1024, num_fine=1024, grid_size=2, **kwargs):
        super().__init__()
        if kwargs:
            logging.warning(f"kwargs: {kwargs} are not used in {__class__.__name__}")
...
class NodeShuffle(nn.Module):
    ...
    def forward(self, xyz, feature, **kwargs):
        ...
        new_xyz = self.proj(feature.squeeze(-1).transpose(1, 2)).view(B, N, -1, 3) + xyz.unsqueeze(2).repeat(
            -1, -1, self.up_ratio, -1
        )
