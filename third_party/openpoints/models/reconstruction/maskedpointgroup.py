...
class MaskedPointGroup(nn.Module):
    ...
    def group_random_masking(x, f=None, mask_ratio=0.9):
        ...
        if f is not None:
            f_masked = torch.gather(
                f,
                dim=2,
                index=ids_keep.unsqueeze(1).unsqueeze(-1).expand(-1, f.shape[1], -1, f.shape[-1]),
            )
        else:
            f_masked = None
    ...
    def forward(self, p, f=None):
        ...
        dp_masked, gf_masked, mask, ids_restore, idx_keep = self.group_random_masking(dp, gf, self.mask_ratio)
