...
class BAAFNet(nn.Module):
    def __init__(self, n_points=4096, ds_ratio=4, k=16, num_classes=13, dims=None, **kwargs):
        """
        ...
        self.EncoderBCBModules = nn.ModuleList(
            [BilateralContextBlock(self.dims[i] * 2, self.dims[i + 1], k=k) for i in range(len(self.dims) - 1)]
        )
        # Decoders
        self.DecoderMLPModules = nn.ModuleList(
            [MLP1d(2 * self.dims[-1 - i], 2 * self.dims[-1 - i]) for i in range(self.num_layers)]
        )
        self.DecoderReconModules = self._getDecoderReconModules()
        self.DecoderWeightModules = nn.ModuleList(
            [MLP1d(2 * self.dims[0], 1, bn=False, activation=None) for i in range(self.num_layers)]
        )
        # Classifier
