...
class PointVectorPartDecoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.aggr_args = kwargs.get("aggr_args", {"feature_type": "dp_fj", "reduction": "max"})
        if self.cls_map == "curvenet":
            ...
            self.convc = nn.Sequential(create_convblock1d(50, 64, norm_args=None, act_args=act_args))
            skip_channels[0] += 64  # shape categories labels
            self.cls2partembed = cls2partembed
    ...
    def forward(self, p, f, cls_label):
        ...
        for i in range(-1, -len(self.decoder), -1):
            f[i - 1] = self.decoder[i][1:]([p[i - 1], self.decoder[i][0]([p[i - 1], f[i - 1]], [p[i], f[i]])])[1]
