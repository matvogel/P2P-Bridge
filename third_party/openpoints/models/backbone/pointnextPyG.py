...
from ..layers import CHANNEL_MAP, create_act, create_linearblock, random_sample
    ...
    def __init__(self, mlp, upsample=True, norm_args={"norm": "bn1d"}, act_args={"act": "relu"}):
        ...
        if not upsample:
            ...
            for i in range(1, len(mlp) - 1):
                linear1.append(create_linearblock(mlp[i], mlp[i + 1], norm_args=norm_args, act_args=act_args))
            self.linear1 = nn.Sequential(*linear1)
        else:
            convs = []
            for i in range(len(mlp) - 1):
                convs.append(create_linearblock(mlp[i], mlp[i + 1], norm_args=norm_args, act_args=act_args))
            self.convs = nn.Sequential(*convs)
    ...
    def forward(self, p, f, b):
        for i in range(-1, -len(self.decoder) - 1, -1):
            f[i - 1] = self.decoder[i][1:](
                [
                    p[i],
                    self.decoder[i][0]([p[i - 1], f[i - 1], b[i - 1]], [p[i], f[i], b[i]]),
                ]
            )[1]
