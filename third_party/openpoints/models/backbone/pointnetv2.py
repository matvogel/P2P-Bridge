...
from ..layers import create_convblock1d, furthest_point_sample, random_sample, three_interpolation
...
class PointNetSAModuleMSG(nn.Module):
    ...
    def forward(
        ...
        for i in range(self.blocks):
            new_features = self.local_aggregations[i](query_xyz, support_xyz, support_features, query_idx=idx)
...
class PointNetFPModule(nn.Module):
    ...
    def forward(
        ...
        if unknow_feats is not None:
            new_features = torch.cat([unknow_feats, interpolated_feats], dim=1)  # (B, C2 + C1, n)
        else:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for k in range(fp_mlps.__len__()):
            ...
            self.FP_modules.append(PointNetFPModule([pre_channel + skip_channel_list[k]] + fp_mlps[k]))
...
class PointNet2PartDecoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        num_samples = self._to_full_list(num_samples, self.blocks, param_scaling=nsample_scaling)
        self.radius = radius
        self.num_samples = num_samples
