...
def furthest_point_sample(coords, num_samples, rgb=None, features=None):
    ...
    return_centers = [centers_coords]
    if rgb is not None:
        center_normals = gather(rgb, indices)
        return_centers += [center_normals]
    if features is not None:
        centers_features = gather(features, indices)
        return_centers += [centers_features]

    if len(return_centers) == 1:
        return return_centers[0]
