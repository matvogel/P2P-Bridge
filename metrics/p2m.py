...
class _FacePointDistance(Function):
    ...
    def backward(ctx, grad_dists):
        ...
        points, tris, idxs = ctx.saved_tensors
        min_triangle_area = ctx.min_triangle_area
        grad_points, grad_tris = _C.face_point_dist_backward(points, tris, idxs, grad_dists, min_triangle_area)
        return grad_points, None, grad_tris, None, None, None
...
class _PointEdgeDistance(Function):
    ...
    def forward(ctx, points, points_first_idx, segms, segms_first_idx, max_points):
        ...
        Returns:
            ...
            spanned by `(v0, v1)`.
        """
        dists, idxs = _C.point_edge_dist_forward(points, points_first_idx, segms, segms_first_idx, max_points)
    ...
    def backward(ctx, grad_dists):
        ...
        grad_points, grad_segms = _C.point_edge_dist_backward(points, segms, idxs, grad_dists)
...
class _EdgePointDistance(Function):
    ...
    def forward(ctx, points, points_first_idx, segms, segms_first_idx, max_segms):
        ...
        Returns:
            ...
            spanned by `(v0, v1)`.
        """
        dists, idxs = _C.edge_point_dist_forward(points, points_first_idx, segms, segms_first_idx, max_segms)
        ctx.save_for_backward(points, segms, idxs)
        return dists
    ...
    def backward(ctx, grad_dists):
        grad_dists = grad_dists.contiguous()
        points, segms, idxs = ctx.saved_tensors
        grad_points, grad_segms = _C.edge_point_dist_backward(points, segms, idxs, grad_dists)
        return grad_points, None, grad_segms, None, None
...
def point_mesh_edge_distance(meshes: Meshes, pcls: Pointclouds):
    ...
    point_to_edge = point_edge_distance(points, points_first_idx, segms, segms_first_idx, max_points)
    ...
    edge_to_point = edge_point_distance(points, points_first_idx, segms, segms_first_idx, max_segms)
...
def point_mesh_face_distance_custom(
    meshes: Meshes,
    pcls: Pointclouds,
